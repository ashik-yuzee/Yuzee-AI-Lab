...
const activeCases=focused?cases.filter(c=>['beginner-skill','readiness-unknown'].includes(c[0])):cases;
async function run(c:readonly string[]){
 const [id,toolId,message]=c;
 try{
 const conv=await(await fetch('http://127.0.0.1:3000/api/conversations',{method:'POST',headers,body:JSON.stringify({title:'Counselling clarity - '+id+' (synthetic)',model:'gemini-3.7-flash',mode:'AUTO',responseMode:'standard',thinkingLevel:'adaptive'})})).json();
 ...
 fs.writeFileSync(focused?'../outputs/Counselling-Clarity-Focused.json':retest?'../outputs/Counselling-Clarity-Final-Checks.json':'../outputs/Counselling-Clarity-Live-Checks.json',JSON.stringify(results,null,2));
}
