...
const results:any[]=[];
function save(){fs.writeFileSync('../outputs/Structured-Teaching-Reviewed-Checks.json',JSON.stringify(results,null,2));}
...
const selected=cases.filter(([id])=>['records','communication','course','comparison','requirements','cost','skills-map','ordinary-chat','brief'].includes(id));
