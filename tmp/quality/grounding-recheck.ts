...
const cases=[
 ...
 ['skills', '@Oala I want to learn spreadsheets so I can keep customer records for a small office. I can type a list but I do not understand what makes it accurate. Help me get started.'],
...
const results:any[]=[];
for(const [id,message] of cases.filter(([id])=>id==='learning-value')){
 ...
 const response=await fetch(`http://127.0.0.1:3000/api/conversations/${conv.id}/messages`,{method:'POST',headers,body:JSON.stringify({message,model:'gemini-3.7-flash',mode:'AUTO',useMultiTurn:true,useStructuredOutput:true,thinkingLevel:'adaptive',maxOutputTokens:id==='output-limit'?128:8192,microToolSelection:{status:'selected',toolId:id==='skills'?'SKILL_009':'COURSE_005',score:.7,margin:.2,reason:'clear-semantic-match',version:'minilm-guarded-v1'}}),signal:AbortSignal.timeout(60000)});
 ...
 results.push(result);fs.writeFileSync('../outputs/Learning-Quality-Grounding-Recheck.json',JSON.stringify(results,null,2));
