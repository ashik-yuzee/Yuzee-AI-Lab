...
const md=`# BGE in Yuzee: tuning results and handover
...
|---|---:|---:|---|
| Pathway decisions | 12/16 correct | **${s.pathwayCorrect}/16 correct** | Improved relevance checks; one missed relevant request remains |
| Input-needs decisions | 12/16 correct | **${s.needsCorrect}/16 correct** | Improved answer / research / clarification classification; three research requests abstain |
| Skill routing | 40 correct, 1 wrong, 62 abstain | **${s.correct} correct, ${s.wrong} wrong, ${s.abstained} abstain** | Previous coverage and error count preserved |
| Best-ranked skill before thresholds | 83/103 | **${s.top1}/103** | Ranking quality is not the same as an accepted correct route |
| Useful follow-up suggestions | 8/24 responses | **${s.suggestionsWithRelevant}/24 responses** | Zero incorrect suggestions in this browser set; coverage remains limited |
| Unrelated requests | All 24 rejected | **All 24 rejected** | No selected route and no suggestion for these examples |
| Additional input-needs cases | New regression set | **${s.newNeedsCorrect}/18 correct** | Zero wrong accepted hints; three abstentions on in-scope requests |
| Additional pathway cases | New regression set | **${s.newPathwaysCorrect}/16 correct** | Zero wrong accepted hints; one relevant request abstains |
...
- **360 browser worker requests** ran through the actual shared worker. Mixed-workload warm latency was about ${s.medianMs.toFixed(1)} ms median, ${s.p95Ms.toFixed(1)} ms at the 95th percentile and ${s.p99Ms.toFixed(1)} ms at the 99th percentile on this computer. Queue priority and cancellation passed. The first worker initialization took ${(s.coldMs/1000).toFixed(1)} seconds with model assets already cached; this is not a cold-network download test.
...
`;
await fs.writeFile('docs/BGE-Single-Model-Update.md',md);
const body=renderToStaticMarkup(React.createElement(ReactMarkdown,{remarkPlugins:[remarkGfm]},md));
const html=`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Yuzee • BGE tuning results</title><style>
:root{color-scheme:light;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#202339;background:#f8f7fc}*{box-sizing:border-box}body{margin:0}main{max-width:1050px;margin:0 auto;background:white;padding:48px 52px 80px}header{color:#7142c0;font-size:13px;letter-spacing:.1em;text-transform:uppercase;border-bottom:1px solid #e8e0f6;padding-bottom:20px;margin-bottom:30px}h1{font-size:36px;line-height:1.18;letter-spacing:-.025em;margin:0 0 18px}h2{font-size:23px;line-height:1.4;margin:42px 0 14px;color:#382454}p,li{font-size:16px;line-height:1.75}p{margin:14px 0}li{margin:9px 0}ul,ol{padding-left:25px}a{color:#673ab1;text-underline-offset:3px}strong{font-weight:650}table{border-collapse:collapse;font-size:14px;line-height:1.55;width:100%;margin:20px 0;display:block;overflow-x:auto}th{text-align:left;background:#f3eefb;color:#43266c}th,td{padding:13px 14px;border-bottom:1px solid #e5e2ec;vertical-align:top}td:first-child{font-weight:550;min-width:170px}pre{background:#f5f3f8;border-left:3px solid #9770d0;padding:20px;overflow:auto;font-size:13px;line-height:1.65}code{font-family:ui-monospace,SFMono-Regular,monospace}footer{border-top:1px solid #e8e0f6;margin-top:36px;padding-top:18px;color:#697087;font-size:13px}@media(max-width:650px){main{padding:28px 20px}h1{font-size:29px}p,li{font-size:15px}th,td{padding:10px}}@media print{main{padding:0;max-width:none}body{background:white}h2{break-after:avoid}tr{break-inside:avoid}}
</style></head><body><main><header>Yuzee · Engineering handover</header>${body}<footer>Measured locally on 17 September 2026. Calibration results are not production certification.</footer></main></body></html>`;
await fs.writeFile('output/html/Yuzee-BGE-Tuning-Results.html',html);
console.log('Created BGE handover report in Markdown and HTML.');
