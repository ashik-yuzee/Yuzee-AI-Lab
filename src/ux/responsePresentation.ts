...
export function responseToReadableText(value: unknown): string {
  ...
  for (const block of response.content_blocks) {
    ...
    for (const row of block.rows || []) parts.push([row.criteria, (row.cells || []).map(cell => `${block.columns.find(c => c.key === cell.key)?.label || cell.key}: ${cell.value || 'Not provided'}`).join('\n')].filter(Boolean).join('\n'));
  }
